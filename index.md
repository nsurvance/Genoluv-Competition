<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Genoluv Art Competition Rules</title>
    <style>
        :root {
            --background: #36393F;
        }

        * {
            margin: 0;
            /* padding: 0; */
            border: 0;
            outline: 0;
            vertical-align: baseline;
            background: transparent;
            font-family: sans-serif;
            /* background-image: linear-gradient(#599FFD, #8EE4F1); */
            background-color: #202225;
            line-height: 150%;
        }

        html,
        body {
            max-width: 100%;
            overflow-x: hidden;
        }

        #wrapper {
            width: 100%;
            max-width: 600px;
            padding-top: 20px;
            border: hidden;
            border-radius: 20px;
            background: var(--background);
            margin: auto;
            margin-top: 40px;
            margin-bottom: 40px;

        }

        #header {
            background: var(--background);
        }

        img {
            width: 90%;
            display: block;
            margin-left: auto;
            margin-right: auto;
            background: var(--background);
        }

        .body {
            margin: auto;
            width: 90%;
            max-width: 400px;
            padding: 10px;
            background: var(--background);
            color: lightgray;
        }

        p {
            padding-bottom: 20px;
            font-size: 20px;
            background: var(--background);
        }

        i {
            background: var(--background);
            color: white;
        }

        b {
            background: var(--background);
            color: white;
        }

        h1 {
            font-size: 30px;
            background: var(--background);
            padding-bottom: 20px;
            padding-top: 20px;
            color: white;
        }

        span {
            background: var(--background);
            font-size: 25px;
        }

        ol {
            background: var(--background);
        }

        li {
            background: var(--background);
            padding-bottom: 20px;
        }

        #logo {
            display: block;
            width: 170px;
            border-radius: 50%;
            margin-top: 40px;
        }

        a {
            color: white;
            background: var(--background);
            text-decoration: none;
        }

        a:hover {
            color: lightblue;
            text-decoration: underline;
        }

        a:visited {
            color: gray;
        }

        a p {
            margin: auto;
            justify-content: center;
            text-align: center;
            padding-top: 20px;
        }
    </style>
</head>

<body>

    <!-- Wrapper -->
    <div id="wrapper">

        <div id="header">
            <img src="image1.png" alt="The Wild Wild West">
        </div>

        <div id="intro" class="body">
            <p><i>Cowboys. Sand. Old towns. And stand offs.</i> Everyone’s familiar with the wild west. But what's
                wilder than
                the
                wild west? The <b>WILD wild west!</b><br />
                Welcome to the first Genoluv Art Competition. For this competition, your prompt is to take the classic
                wild
                west and put your own creative spin on it! Essentially, make it even <i>wilder</i> than it already is,
                in any
                way
                you like!</p>

            <p>Start: <b>Tuesday, June 1st, 2021</b><br />
                Deadline: <b>Tuesday, June 22nd, 2021</b><br />
                Winner’s announced: <b>Wednesday, June 30th, 2021</b></p> <br />
        </div>

        <div id="compete" class="body">
            <h1>But Who Can Compete?</h1>
            <p><span>EVERYONE!!</span> (except Helpers, Moderators, and the Owners)</p>
        </div>

        <div id="entry" class="body">
            <h1>How do I Enter?</h1>
            <p>Entry is simple! You’ll be given three weeks (count ‘em, 3!) to create your own amazing
                illustration.<br /><br />
                After that, privately message a Helper, Moderator, or Owner your finished piece and that’s that!
                <br /><br />
                We will be offering a <i>Normal</i> and <i>Beginner</i> entry level. <i>Beginner</i> is for
                <b>Beginners</b>, so it’s up to
                moderator discretion which entry level you’ll be entered at based upon what the moderators as a whole
                feel is appropriate.
            </p>
        </div>

        <div id="restrictions" class="body">
            <h1>Are There any Restrictions or Requirements?</h1>
            <p>Yes! To be qualified to enter you must meet the following guidelines:</p>
            <ol>
                <li><b>You must have a background!</b> You are given 3 weeks to work on this AND you have the potential
                    to win
                    real money! We expect some fully realized pieces of work!</li>
                <li><b>The minimum is shading!</b> We understand not everyone has access to good color materials, but we
                    do
                    expect some level of shading with discretion for personal style. Essentially, <b>NO LINE-ART
                        ONLY.</b></li>
                <li><b>Special Details!</b> Part of the fun of a competition is requiring something to see how artists
                    creatively think around it. For this competition, we want to see a <b>Cactus with a Cowboy Hat</b>
                    somewhere in the image. On a pin? On a hat? Carved in a tree? Or just part of the background? THE
                    MAIN CHARACTER??? It’s your choice, but make it happen! Be creative! Make it visible!</li>
                <li><b>No NSFW works!</b> We want to showcase this to the whole server, so we need it to be squeaky
                    clean. Keep
                    it PG-13 at MAX.</li>
            </ol>
        </div>

        <div id="prize" class="body">
            <h1>Cool, but what’s in it for me?</h1>
            <p><b>
                    <i>PRIZES!!</i>
                </b> Now considering this is the first ever competition, we don’t have an insanely large prize pool
                to offer but we hope what we do offer is enough to pique your interests!
            </p>

        </div>
        <img src="image 2.JPG" alt="Potential Prizes">

        <div id="judge" class="body">
            <h1>How Will These be Judged?</h1>
            <p><i>With the power of democracy (Sorta)!</i> The Moderators will judge all the submitted pieces until we
                have our finalists. There the rest of server has the opputunity to vote on who wins the final prize!
                There can only be one vote
                per person. <br /><br />We will also be providing this rubric to help guide voters (and mods) to making
                an
                intelligent
                judgement.</p>

        </div>
        <img src="image3.JPG" alt="Rubric">

        <div id="rules" class="body">
            <h1>Any Other Rules I Should Know About?</h1>
            <p>Yes, actually, and please follow them carefully!</p>
            <ol>
                <li><b>Submit by The Deadline.</b> Sounds easy enough, doesn’t it? No late work will be accepted.</li>
                <li><b>Do not post your submissions on the server.</b> We want to avoid people potentially stealing
                    ideas or
                    being discouraged by other people’s works! The mods will showcase them all at the end in a special
                    channel!</li>
                <li><b>The last winner can’t win again!</b> We don’t want chain-winners. So winners from the previous
                    competition can not win again. Feel free to enter though!</li>
                <li><b>Follow all the rules and requirements!</b> Sounds easy, dunnit?</li>
            </ol>
            <p>That’s everything guys! We hope to see your future works! :)<br />
                Please note that any rules are subject to change in the future.
            </p>

            <a href="https://discord.com/invite/D9xyUec"><img id="logo" src="gif.gif" alt="Genoluv Art">
                <p>Join Genoluv Art on Discord!</p>
            </a>
        </div>
    </div>


</body>

</html>